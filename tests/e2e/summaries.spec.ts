...
test("summaries show totals by vehicle, category and month including zero totals", async ({ page }) => {
  ...
  await page.getByLabel("Apelido").fill("Carro Sumário A");
  ...
  await page.getByLabel("Apelido").fill("Carro Sumário B");
  ...
  await createForm.getByLabel("Veículo").selectOption({ label: "Carro Sumário A (Toyota Corolla)" });
  ...
  const carA = page.locator("[data-testid='summary-card']").filter({
    has: page.getByRole("heading", { name: "Carro Sumário A (Toyota Corolla)" }),
  });
  ...
  const carB = page.locator("[data-testid='summary-card']").filter({
    has: page.getByRole("heading", { name: "Carro Sumário B (Honda Fit)" }),
  });
